import { Box, Stack, Typography, Avatar, useTheme } from "@mui/material";
import some from "./some.png";
import SignIn from "./SignIn";
import { useState, useEffect } from "react";
import ZedGoStyles from "../CSS/ZedGo/ZedGoTheme";

const ZedGo = () => {
  const [showSignUp, setShowSignUp] = useState(false);
  const theme = useTheme();
  const styles = ZedGoStyles(theme);

  useEffect(() => {
    const timer = setTimeout(() => {
      setShowSignUp(true);
    }, 1000);

    return () => clearTimeout(timer);
  }, []);

  if (showSignUp) {
    return <SignIn />;
  }

  return (
    <Box sx={styles.mainContainer}>
      <Box sx={styles.contentBox}>
        <Stack sx={styles.stackContainer}>
          <Avatar sx={styles.avatarStyle} alt="Remy Sharp" src={some} />
          <Typography sx={styles.typographyMain}>ZedGoo</Typography>
        </Stack>
        <Typography sx={styles.typographySub}>connecting...</Typography>
      </Box>
    </Box>
  );
};

export default ZedGo;


// import { Box, Stack, Typography, Avatar } from "@mui/material";
// import some from './some.png'
// import SignIn from "./SignIn";
// import { useState, useEffect } from "react";

// const ZedGo = () => {
//   const [showSignUp, setShowSignUp] = useState(false);

//   useEffect(() => {
//     const timer = setTimeout(() => {
//       setShowSignUp(true);
//     }, 1000);

  
//     return () => clearTimeout(timer);
//   }, []);


//   if (showSignUp) {
//     return <SignIn />;
//   }

  
//   return (
//     <Box
//       sx={{
//         display: "flex",
//         flexDirection: "column",
//         justifyContent: "center",
//         alignItems: "center",
//         backgroundColor: "GrayText",
//         height: "100vh",
//       }}
//     >
//       <Box sx={{ color: "white" }}>
//         <Stack
//           sx={{
//             display: "flex",
//             flexDirection: "row",
//           }}
//         >
//           <Avatar sx={{ width: "70px", height: "70px" }} alt="Remy Sharp" src={some} />
//           <Typography fontSize={50}>ZedGoo</Typography>
//         </Stack>
//         <Typography ml={20}>connecting...</Typography>
//       </Box>
//     </Box>
//   );
// };

// export default ZedGo;
