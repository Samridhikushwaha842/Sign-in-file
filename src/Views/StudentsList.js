...
import { Avatar, Box, IconButton, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, Typography, Grid, Button, useTheme } from '@mui/material';
import { Search, Visibility, VisibilityOff } from '@mui/icons-material';
import StudentsListStyles from '../CSS/StudentsList/StudentsListTheme';
...
const StudentsList = ({ students, onViewClick, viewedStudentId }) => {
  ...
  const styles = StudentsListStyles(theme);
  ...
  return (
    <Box sx={styles.container}>
        ...
        <Table>
          ...
          <TableBody>
            {filteredStudents.map((student, index) => (
              <TableRow key={student.id} sx={index % 2 === 0 ? styles.tableRowEven : styles.tableRowOdd}>
                <TableCell sx={styles.cellStyle}>
                  <Avatar alt={student.name} src={student.profile} sx={styles.avatar} />
                </TableCell>
                <TableCell sx={styles.cellStyle}>{student.name}</TableCell>
                <TableCell sx={styles.cellStyle}>{maskEnrolmentNo(student.enrolmentNo)}</TableCell>
                <TableCell sx={styles.cellStyle}>{student.mobile}</TableCell>
                <TableCell sx={styles.cellStyle}>{student.email}</TableCell>
                <TableCell sx={styles.cellStyle}>
                  <IconButton onClick={() => onViewClick(student.id)}>
                    {viewedStudentId === student.id ? <VisibilityOff sx={styles.eyeStyle} /> : <Visibility sx={styles.eyeStyle} />}
                  </IconButton>
                </TableCell>
              </TableRow>
            ))}
      ...
      <Box sx={styles.loadMoreButton}>
        <Button underline>Load More</Button>
      </Box>
    </Box>
  );
};
